...
public class GrpcClient {
    ...
    private static void testBlocking (int retries, ExecutorService pool, Channel channel) throws InterruptedException {
        if (retries <= 0) {
    ...
    private static void testNonBlocking (int retries, ExecutorService pool, Channel channel) throws InterruptedException {
        if (retries <= 0) {
            throw new IllegalArgumentException();
        }
        Semaphore counter = new Semaphore(0);
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Channel nonBlockingChannel = NettyChannelBuilder.forAddress("localhost", 80).usePlaintext().build();
        ...
        Channel blockingChannel = ManagedChannelBuilder.forAddress("localhost", 80).usePlaintext().build();
